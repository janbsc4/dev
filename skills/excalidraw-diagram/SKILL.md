...
**Setup:** If the user asks you to set up this skill (renderer, dependencies, etc.), follow the **First-Time Setup** instructions in the Render & Validate section below.
...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. This is a hard constraint — coding agents have a per-response output limit (~32,000 tokens), and a comprehensive diagram easily exceeds that in one shot. Even if it didn't, generating everything at once leads to worse quality. Section-by-section is better in every way.
...
```bash
cd <skill-directory>/references && uv run python render_excalidraw.py <absolute-path-to-file.excalidraw>
```

`<skill-directory>` is the folder containing this SKILL.md (e.g. `~/.claude/skills/excalidraw-diagram`, `~/.config/opencode/skills/excalidraw-diagram`, or wherever your agent installed the skill). This outputs a PNG next to the `.excalidraw` file. Then view the PNG with your file/image reading tool to actually see it.
